...
class MetDataLoader:
    ...
    def __call__(self, sample: dict):
        ...
        sample["case"] = int(str(data_path).split("set.")[1].split(".")[0])
...
class MetDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        sample = {"_idx": idx, "_mean": self.mean, "_std": self.std, "_topo": self.topo, "_label_mean": self.label_mean, "_label_std": self.label_std, "_weight_per_class": self.weight_per_class}
...
def load_data(
    ...
    datasets = []
    datasets.append(MetDataset(mean=mean, std=std, topo=topo, label_mean=label_mean, label_std=label_std, weight_per_class=weight_per_class, episode_paths=filepaths, transform_pipeline=transform_pipeline))
