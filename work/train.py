...
def train(
    ...
    **kwargs,
    ...
    weight_file = '../dataset/lgt_weight.npz'
    weight_per_class = np.load(weight_file)['weight_per_class']
    ...
    if (mode == "scratch"):
        ...
        optimizer_gen = torch.optim.AdamW(model_gen.parameters(), lr=lr, weight_decay=0.01)
        optimizer_gen = load_optimizer('regressor', optimizer_gen)
        for param_group in optimizer_gen.param_groups:
            param_group['lr'] = lr # Update each parameter group

        optimizer_dis = torch.optim.AdamW(model_dis.parameters(), lr=lr, weight_decay=0.01)
        optimizer_dis = load_optimizer('discriminator', optimizer_dis)
    ...
    for epoch in range(num_epoch):
        ...
        for data in train_data:
            img, label, mask, detected, lgt, weights = data['image'], data['label'], data['mask'], data['detected'], data['lgt'], data['weights']
            img, label, mask, detected, lgt, weights = img.to(device), label.to(device), mask.to(device), detected.to(device), lgt.to(device), weights.to(device)
            label = label.type(torch.float32)

            # Train Discriminator
            optimizer_dis.zero_grad()
            pred, logit, logit_lgt = model_gen(img)
            pred = pred.squeeze()
            logit = logit.squeeze()
            logit_lgt = logit_lgt.squeeze()
            
            real_dis = model_dis(label[:,None,:,:], img).squeeze()
            fake_dis = model_dis(pred[:,None,:,:] * mask[:,None,:,:], img).squeeze()

            dims = (fake_dis.size(0), fake_dis.size(1), fake_dis.size(2))

            real_loss = loss_func_dis(real_dis, torch.ones(*dims, device=device))
            fake_loss = loss_func_dis(fake_dis, torch.zeros(*dims, device=device))
            ...
            adv_loss = loss_func_dis(fake_dis, torch.ones(*dims, device=device))
            pix_loss = loss_func_gen(pred, logit, logit_lgt, label, mask, detected, lgt, weights)
            print(adv_loss, pix_loss)
            loss_gen = adv_loss + 50 * pix_loss
            loss_gen.backward()
            optimizer_gen.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_dir", type=str, default="multihead_logs")
